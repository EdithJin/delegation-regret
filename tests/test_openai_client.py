...
def pinned_client(base_url: str) -> OpenAIClient:
    """The chat-completions client at the leg's published constants. Since the
    2026-08-27 move to /v1/responses, gpt-5.6-sol itself dispatches to
    OpenAIResponsesClient (tests/test_responses_client.py); this client stays
    for the OpenAI-compatible open-source path, and effort "high" here
    exercises the reasoning_effort parameter that path still carries."""
